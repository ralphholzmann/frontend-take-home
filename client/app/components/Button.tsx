...
const variants: Record<ButtonVariant, { default: string, disabled: string }> = {
  ...
  secondary: {
    default: "bg-background text-foreground border-grays-7 cursor-pointer hover:border-grays-6 active:border-grays-6 hover:bg-grays-2 active:bg-background",
    disabled: 'bg-gray-alpha-3 border-transparent text-gray-alpha-8 cursor-not-allowed'
  },
  danger: {
    default: "bg-red-surface border-red-alpha-7 text-red-alpha-11 cursor-pointer hover:bg-red-surface-hover active:bg-red-surface hover:border-red-divider active:border-red-divider",
    disabled: 'cursor-not-allowed'
  }
}
...
const Button = ({ children, variant = "primary", size = "md", className, iconStart, type="button", disabled = false, ...props }: ButtonProps) => {
